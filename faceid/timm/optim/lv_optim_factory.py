...
def lv_add_weight_decay(model, weight_decay=1e-5, skip_list=()):
    [backbone, head] = model
    decay = []
    no_decay = []
...
def create_optimizer_v2(
    ...
    elif isinstance(model_or_params, list):
        [backbone, head] = model_or_params
        if weight_decay and filter_bias_and_bn:
            skip = {}   # only consider backbone
            if hasattr(backbone, 'no_weight_decay'):
                skip = backbone.no_weight_decay()
            parameters = lv_add_weight_decay(model_or_params, weight_decay, skip)
