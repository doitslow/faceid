...
def convert_splitbn_model(module, num_splits=2):
    """
    Recursively traverse faceid and its children to replace all instances of
    ``torch.nn.layer.batchnorm._BatchNorm`` with `SplitBatchnorm2d`.
    Args:
        module (torch.nn.Module): input faceid
        num_splits: number of separate batchnorm layers to split input across
    Example::
        >>> # model is an instance of torch.nn.Module
        >>> model = timm.model.convert_splitbn_model(model, num_splits=2)
    """
    mod = module
