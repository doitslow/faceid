...
# class FaceTransformerDataset(data.Dataset):
#     def __init__(self, path_imgrec, rand_mirror):
class FaceTransformerDataset(data.Dataset):
    def __init__(self, info: dict, rand_mirror=False):
        self.rand_mirror = rand_mirror
        path_imgrec = os.join(info['data_dir'], info['rec_path'])
        assert os.path.exists(path_imgrec)
        if path_imgrec:
            print('loading recordio %s...', path_imgrec)
...
if __name__ == '__main__':
    ...
    embed()
    dataset = FaceTransformerDataset(path_imgrec =root, rand_mirror = False)
